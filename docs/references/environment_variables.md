...
| `SGLANG_API_LOGLEVEL` | Controls crash-debug API logging. `0` disables logging, `1` logs API names, `3` logs tensor metadata, `5` adds tensor statistics, and `10` also writes pre-call dump snapshots. | `0` |
| `SGLANG_API_LOGDEST` | Destination for crash-debug API logs. Use `stdout`, `stderr`, or a file path. `%i` is replaced with the process PID. | `stdout` |
| `SGLANG_API_DUMP_DIR` | Output directory for level-10 API input/output dumps. `%i` is replaced with the process PID. | `sglang_api_dumps` |
| `SGLANG_API_DUMP_INCLUDE` | Comma-separated wildcard patterns for API names to include in level-10 dumps. | Not set |
| `SGLANG_API_DUMP_EXCLUDE` | Comma-separated wildcard patterns for API names to exclude from level-10 dumps. | Not set |
