...
The refactor this belongs to exists because the opposite was true: fourteen
probes were imported at module scope by six hundred and forty-seven places, so
"this box is SM100" could only be said to one reader at a time. A round-4 CI
failure came from exactly that -- the module that chose an attention backend and
the module that validated the choice held different copies, and a patch moved
only one of them.
"""
