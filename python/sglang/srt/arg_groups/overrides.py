...
# The declarations one directory down (``model_overrides/``) are written
# against these, and import them from ``model_override_base`` directly. The
# re-export keeps every existing ``from ...overrides import x`` working.
...
def pre_capture_activation_reserve_mb_of(cfg: Any, gpu_mem: Optional[float]) -> float:
    ...
    two equal. The rule used to be written out a second time on the record
    instead of living here, which made it the only one of that group with two
    implementations rather than a helper and a delegate.
    """
...
def modelexpress_config_of(cfg: Any) -> dict:
    ...
    own, so everything that wants a key out of it goes through here. The record
    used to carry a second, instance-cached copy of this parse and the two
    spellings had to be kept in step by hand.
    """
...
def collect_model_override_declarations(
    ...
    """
    # Read the three registries off the module rather than through the names
    # imported above. ``register_model_override_predicate`` appends to the
    # base's list; a copied name here is a second binding, and anything that
    # rebinds one of them (a test isolating the registry) would have the
    # registrar writing to one list while this reads the other.
